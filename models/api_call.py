...
class ApiCall(models.Model):
    ...
    state = fields.Selection(selection_add = [('api_call', 'Call External API')], ondelete={'api_call': 'cascade'})
    ...
    payload = fields.Text('payload', render_engine='qweb', translate=True, sanitize=False)
    ...
    def _run_action_api_call(self, eval_context=None):
        ...
        raise UserError(api_result.json().get('recived data').get('note').get('to')[0])
